...
def test_create_post_default_published_is_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "newtitle", "content": "newcontent"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "newtitle"
    assert created_post.content == "newcontent"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "new title", "content": "abcd"})
    assert res.status_code == 401
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(f"/posts/560000")
    assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    data = {"title": "updated title", "content": "updated content", "id": test_posts[3].id}
    res = authorized_client.put(f"/posts/12345", json=data)
    assert res.status_code == 404